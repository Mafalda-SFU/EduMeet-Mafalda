...
const configSchema = convict({
	...
	mediasoupVertical : {
		appData : {
			doc     : 'Custom application data.',
			format  : Object,
			default : {}
		},
		dtlsCertificateFile : {
			doc     : 'Path to the DTLS public certificate file in PEM format. If unset, a certificate is dynamically created.',
			format  : String,
			default : ''
		},
		dtlsPrivateKeyFile : {
			doc     : 'Path to the DTLS certificate private key file in PEM format. If unset, a certificate is dynamically created.',
			format  : String,
			default : ''
		},
		logLevel : {
			doc     : 'Logging level for logs generated by the media worker subprocesses (check the Debugging documentation). Valid values are “debug”, “warn”, “error” and “none”.',
			format  : String,
			default : 'error'
		},
		logTags : {
			doc     : 'Log tags for debugging. Check the list of available tags in Debugging documentation.',
			format  : Array,
			default : []
		},
		maxWorkers : {
			doc     : 'The maximum number of Mediasoup workers to spawn.',
			format  : Number,
			default : Infinity
		},
		rtcMaxPort : {
			doc     : 'Maximum RTC port for ICE, DTLS, RTP, etc.',
			format  : Number,
			default : 59999
		},
		rtcMinPort : {
			doc     : 'Minimun RTC port for ICE, DTLS, RTP, etc.',
			format  : Number,
			default : 10000
		},
		watermarkNode : {
			doc     :	'CPU percentage watermark to create a new Remote Mediasoup node.',
			format  : Number,
			default : 90
		},
		watermarkWorker : {
			doc     : 'CPU percentage watermark to create a new Mediasoup worker.',
			format  : Number,
			default : 60
		}
	},
